    ...
    public partial class frm_cad_produto : Form
    {
        ...
        private void dg_produto_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txt_desc.Text = row.Cells["descricao_produto"].Value.ToString();
                txt_quantidade.Text = row.Cells["quantidade"].Value.ToString();
                txt_valor.Text = row.Cells["valor_produto"].Value.ToString();
                txt_mini.Text = row.Cells["quant_mini"].Value.ToString();
                txt_maxi.Text = row.Cells["quant_maxi"].Value.ToString();
